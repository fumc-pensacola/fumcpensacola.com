...
function resurrect_add_theme_support_framework() {
	...
	add_theme_support( 'ctfw-after-activation', array(
		...
		'notice'				=> sprintf( __( '<b>Next Steps:</b> Please continue reading the <a href="%s" target="_blank">Getting Started</a> guide for the next steps after theme activation.', 'resurrect' ), 'http://churchthemes.com/guides/user/getting-started/' ),
	...
	add_theme_support( 'ctfw-edd-license', array(
		...
		'expired_notice'			=> __( '<strong>License Expired:</strong> <a href="%4$s" target="_blank">Renew</a> your <a href="%1$s">License Key</a> to re-enable updates and support for the <strong>%2$s</strong> theme (expired on <strong>%3$s</strong>). <a href="%5$s" target="_blank">Learn More</a>', 'resurrect' ), // optional notice to override default with when license is expired
	...
	// WordPress core and its translations sometimes use text that is not preferred (e.g. too long)
 	add_theme_support( 'ctfw-replace-wp-text', array(
        'Correo electrónico'	=> __( 'Email', 'contact_form', 'resurrect' ), // Spanish: too long for comment form
        'Adresse de contact'	=> __( 'Courriel', 'contact_form', 'resurrect' ), // French: too long for comment form
    ) );
	...
	add_theme_support( 'ctfw-widget-archives', array(
		...
		'field_overrides' => array(
			'post_type' => array( // explicitly set post type options that this theme provides an archive for
				'options' => array(
					'post'			=> _x( 'Blog', 'archives widget', 'resurrect' ),
					'ctc_sermon'	=> _x( 'Sermons', 'archives widget', 'resurrect' ),
				),
			),
		),
	) );
